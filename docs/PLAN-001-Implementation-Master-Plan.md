...
- **2026-07-30 — P0.E1.S2.T5 implemented on branch `p0/e1-s2-t5-auto-trade-scope`.** `auto_trade_status_report`'s query scoped via an `EXISTS` join against `premover_auto_log` (`mode='enforce'`, `would_trade=1`) exactly as proposed when the task was added; `yesterday` now uses `datetime.now(WIB)`. 5 new named tests (`tests/test_auto_trade_status_report.py`), each confirmed to fail against the pre-fix code and pass after. Full suite 1,241 passed/1 skipped/0 failed (+5 from baseline 1,236). Gate green. `DEBT-001`/`DEBT-002` close on merge, not before. Evidence bundle complete; cold-reviewed as an independent pass (1 Minor doc-wording finding, fixed before merge; adversarial edge-case probing found no functional defects) and merged to `master`.
- **2026-07-30 — P0.E1.S2.T6 implemented on branch `p0/e1-s2-t6-vpin-backfill-register`.** `run_vpin_backfill` registered daily mon-fri at 18:15 WIB (15 min after its data source, `run_vpin_daily_batch` at 18:00) — investigation found it not superseded by the daily batch (which never backfills gaps) and cheap/idempotent to run on a schedule, matching the existing daily cadence of `run_ohlcv_reconciliation`/`run_ohlcv_coverage_check`. `ALLOWLIST` entry in `scripts/audits/an8_unregistered_jobs.py` removed; AN-8 audit now shows 37/37 clean, 0 allowlisted. 3 new named tests; full suite 1,239 passed/1 skipped/0 failed (+3 from the 1,236 baseline this branch forked from). Gate green. Closes `DEBT-003` on merge. Evidence bundle complete; cold-reviewed as an independent pass (0 findings — isolated diff, registration verified exactly-once/correctly-ordered from source, all 3 new tests confirmed to fail pre-fix and pass post-fix) and merged to `master`, reconciled against T5's already-merged doc changes. P0.E1.S2 (dead jobs/reports decision) now fully closed, T1–T6.
