...
|---|---|---|
| **(a) Count N** | ✅ **Yes, for the covered window** | 154 ticker-events (77 ADD / 77 DELETE); 143 distinct economic events; sub-counts by index/direction in [[COVERAGE_REPORT]]. Directly countable from `reconstitution_events.csv`. |
...
2. **A coverage-sufficiency judgment.** The verified window is **~3 years / 154 events (2023-02 → 2026-02)**, not the full ~5 years. This is enough to *compute* N/σ/MDE/power, but whether it is enough to *register on* — versus first closing gaps G-WPD-1…G-WPD-3 for more history and the 2022 bear regime — is a CRO call.
...
> **The required inputs to compute N, σ, MDE, and a ≥80% power analysis are now available for the window 2023-02-01 → 2026-02-02 (154 events).** They are **not** available for the full 5-year window; 2021-H2, 2022, and the May minor-evaluations remain documented gaps, closable only by further retrieval — **not** by imputation.
>
