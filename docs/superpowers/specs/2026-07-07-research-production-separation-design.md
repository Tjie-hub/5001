...
| **M4 — DB split** | Move `wf_scores`, `wf_edge`, `backtest_cache` → `research.db`; production code no longer references them (guaranteed by M1). Archive-in-place first (reversible, per the 2A rebuild idiom). | Grep: zero production references; suite green; dashboards that displayed research stats point at research.db read-only or are marked research-only. |
