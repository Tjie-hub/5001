...
surrounding filters (fundamental/sector/flow/liquidity) switched off and no
OHLCV data for the test ticker, so control reaches the VPIN block and then
exits cleanly (df is None -> continue) regardless of the VPIN verdict —
isolating the VPIN gate's own behavior without needing full scan fixtures.
"""
import logging
...
def wire_common_mocks(monkeypatch):
    ...
    def _set(config=None, db_connect=None, calc_vpin_multi=None):
...
def test_vpin_fails_closed_and_alarms_on_calc_error(wire_common_mocks, monkeypatch):
    ...
    proves the fix isn't narrowly scoped to the one NameError call site."""
    alarm_calls = []
    monkeypatch.setattr(fail_open_alarm_module, "fail_closed_alarm",
                         lambda *a, **k: alarm_calls.append((a, k)))
    ...
    wire_common_mocks(db_connect=lambda _path: _FakeConn(), calc_vpin_multi=_boom)
