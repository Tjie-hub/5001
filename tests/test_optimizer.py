...
def test_run_vol_weighted_returns_expected_keys():
    from research.optimizer import _run_vol_weighted
    df = _make_df(250)
...
def test_run_momentum_returns_expected_keys():
    from research.optimizer import _run_momentum
    df = _make_df(250)
...
def test_run_vwap_reversion_returns_expected_keys():
    from research.optimizer import _run_vwap_reversion
    df = _make_df(250)
...
def test_run_conservative_returns_expected_keys():
    from research.optimizer import _run_conservative
    df = _make_df(250)
...
def test_run_tfb_returns_expected_keys():
    from research.optimizer import _run_tfb
    df = _make_df(250)
...
def test_param_grids_contains_all_five_strategies():
    from research.optimizer import PARAM_GRIDS
...
def test_iter_param_grid_full_product():
    from research.optimizer import _iter_param_grid
    grid = {'a': [1, 2], 'b': [10, 20]}
...
def test_grid_search_returns_sorted_by_sharpe():
    from research.optimizer import grid_search
    df = _make_df(250)
...
def test_grid_search_result_count_matches_grid_size():
    from research.optimizer import grid_search, PARAM_GRIDS
    df = _make_df(250)
    results = grid_search(df, 'conservative', capital=10_000_000)
...
def test_optimize_strategy_returns_expected_keys():
    from research.optimizer import optimize_strategy
...
def test_save_and_get_round_trip(tmp_path):
    from research.optimizer import save_optimizer_result, get_optimizer_result
...
def test_optimizer_run_returns_200_with_best_params(client):
    ...
    with patch('routes.backtest.sqlite3.connect', return_value=mock_conn), \
         ...
         patch('research.optimizer.optimize_strategy', return_value=fake_result), \
         patch('research.optimizer.save_optimizer_result'):
...
def test_optimizer_result_get_returns_404_if_missing(client):
    ...
    with patch('research.optimizer.get_optimizer_result', return_value=None):
...
def test_optimizer_result_get_returns_200_with_cached(client):
    ...
    with patch('research.optimizer.get_optimizer_result', return_value=cached):
