...
- [ ] Every P0 task merged with evidence bundle (`docs/evidence/P0/<task-id>/`) — 7/16 merged (P0.E1.S1.T1, P0.E1.S1.T2, P0.E1.S2.T1, P0.E1.S2.T2, P0.E1.S2.T3, P0.E1.S2.T4, P0.E1.S2.T5); `P0.E1.S2.T6` (T4's own new finding, DEBT-003 — `run_vpin_backfill`) implemented + evidence-complete on branch `p0/e1-s2-t6-vpin-backfill-register`, pending cold review + merge. Denominator stands at 16. Checkbox stays unticked until all 16 land.
- [x] Zero imported-but-unregistered jobs (grep-audit output filed) `[H-1/H-2/AN-8]` — owning task P0.E1.S2.T4, merged 2026-07-30: `scripts/audits/an8_unregistered_jobs.py` checks all 37 scheduler-exported candidates (not just the 6 originally-named ones), 36 clean + 1 allowlisted-with-follow-up (`run_vpin_backfill` → `P0.E1.S2.T6`, `AUDIT-REPORT.md` has full findings). `scripts/pre_merge_gate.py`'s QG-9 now runs it for real (auto-wired per `IMPL-DEC-003`, no gate-script edit needed) and passes. Note: this item's own text is satisfied (grep-audit filed, gate green); the *disposition* of the one allowlisted finding is tracked separately under P0.E1.S2.T6, not a condition of this checkbox.
