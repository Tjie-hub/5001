...
## IMPL-DEC-007 — 2026-07-30 — Test-coverage scope for the freshness guard in `scheduled_multi_strategy_scan` (P0.E2.S1.T2)

**Type:** Implementation decision
**Context:** P0.E2.S1.T2 wires the same `is_fresh()` guard (skip + count + one aggregate warning) into both scan loops named in the task card — `scan_momentum_signals` and `scheduled_multi_strategy_scan` — plus `monitor.py`. `scan_momentum_signals` returns immediately after its main ticker loop, so `tests/test_scanner_freshness_guard.py` can drive the real function end-to-end (fresh/stale/boundary), proving downstream reached via a spy on the first post-guard call, matching the existing `test_scanner_vpin_gate.py` idiom. `scheduled_multi_strategy_scan` does not: after its ticker loop it unconditionally continues into flow-fetch, signal persistence, a second (SELL/distribution) scan pass, and a full-universe bear dip-scout watchlist pass, regardless of whether the loop found anything — each touching tables/network seams this task does not otherwise touch.
**Options considered:**
1. Build the full mock harness anyway, so both scan loops get symmetric end-to-end coverage — cost (roughly a dozen unrelated subsystems to mock: watchlist, distribution scan, signal persistence, flow batch, market-risk/breadth/technicals diagnostics) is disproportionate to a task explicitly framed as "deliberately minimal" (task card intent), and risks ER-12 thinness / scope creep into unrelated subsystems' test surface.
2. Skip test coverage for this call site entirely, relying on code review only.
3. Rely on (a) `engine/freshness.py`'s `is_fresh()` unit tests (`tests/test_freshness_guard.py`) covering the function's own correctness including boundary/weekend/missing-date cases, and (b) `scan_momentum_signals`'s end-to-end proof that the identical guard idiom (`if not is_fresh(df["date"].iloc[-1]): stale_skipped += 1; continue`, placed immediately after the same `len(df) < N` history check) behaves correctly when wired into a real scan loop — verified by direct code inspection that the `scheduled_multi_strategy_scan` insertion is textually the same shape (differs only in `< 25` vs `< 20` and the log-line source tag), not just assumed from the parallel description.
**Choice:** Option 3.
**Reversibility:** trivial — a pure test-scope choice; if this call site's guard ever needs a behavioral fix, targeted coverage can be added then without touching this task's other tests.
**Consequence:** `scheduled_multi_strategy_scan`'s freshness guard has 0 direct integration tests; confidence in it rests on pattern-equivalence with the guard in `scan_momentum_signals`, not independent end-to-end proof. Recorded here so a future reader doesn't mistake the silence for an oversight.

**Update — 2026-07-30 (appended, entry not edited per §8 rule): factual correction found in cold review.** The "SELL/distribution loop shares the same `ohlcv_map` (no second read)" claim in this task's `EVIDENCE.md` §Deliverable is **wrong**. `scheduled_multi_strategy_scan`'s adaptive-selection loop and `scan_distribution_signals()` are two independent functions with two independent `ohlcv_map.get(ticker)` reads — the latter is called separately (`scanner.py:1501`), driven by its own DB query (`stockbit_flow` BEARISH rows) rather than the adaptive-selection loop's ticker set, and had **no freshness guard at all** prior to this correction. This is not a test-coverage-scope question like the rest of this entry — it is the exact H-3 failure mode (a stale last bar evaluated as current) live on a real, unguarded call site, found by independently re-deriving the claim against the source rather than trusting the prose. **Fixed same session:** `is_fresh()` guard added to `scan_distribution_signals()`'s loop (skip + count + one aggregate warning, same idiom as the other three sites), plus 4 new end-to-end named tests in `tests/test_scanner_freshness_guard.py` (fresh/stale/both session boundaries) driving the real function — this call site now has the direct integration coverage this entry says the *other* one deliberately lacks. `tests/test_bearish_signal_path.py`'s pre-existing `scan_distribution_signals` fixtures (fixed 2026-06-05 OHLCV dates) were now-stale relative to the freshness guard and were switched to end on the real current date, same idiom as T2's other fixture updates (`test_monitor_kernel_exits.py` et al.). Full suite: 1,269 passed/1 skipped/0 failed (was 1,265; +4). Evidence updated: `docs/evidence/P0/P0.E2.S1.T2/EVIDENCE.md` §"Cold review".
